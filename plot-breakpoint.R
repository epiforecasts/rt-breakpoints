...
# Get data
data_list <- readRDS(data_list, here::here(Sys.Date(), 
                              "formatted-data.RDS"))
data <- data_list$data
data_ma <- data_list$data_ma

# Get models
models <- readr::read_csv(here::here(Sys.Date(), 
...
# save
saveRDS(plot_breaks, here::here(Sys.Date(), "sw-wales-ni.rds"))

ggsave(filename = here::here(Sys.Date(), "sw-wales-ni.png"),
       height = 14, width = 14)
